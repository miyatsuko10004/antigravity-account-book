...
### 4. 環境変数の設定 (Basic認証)
家族のみに公開するために、Basic認証を設定します。Vercelのダッシュボードで以下の環境変数を設定してください。

- `BASIC_AUTH_USER`: ユーザー名 (例: `family`)
- `BASIC_AUTH_PASSWORD`: パスワード (例: `secret123`)

設定後、再デプロイ（または自動デプロイ）されると、サイトアクセス時に認証が求められるようになります。
